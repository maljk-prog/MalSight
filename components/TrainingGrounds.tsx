...
const domains: { id: SecurityDomain | "mixed"; name: string; description: string }[] = [
  { id:"mixed", name:"Mixed / All Domains", description:"Draw from all eight security domains." },
...
export default function TrainingGrounds() {
  ...
  return <div className="training-shell space-y-6">
    ...
    <section><div className="flex flex-wrap items-end justify-between gap-2"><h3 className="training-section-title">1 / SECURITY DOMAIN</h3><p className="theme-muted text-xs">Choose one or more domains, or use Mixed for all eight.</p></div><div className="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{domains.map((item) => { const selected = selectedDomains.includes(item.id); return <button key={item.id} aria-pressed={selected} onClick={() => chooseDomain(item.id)} className={`training-choice ${selected ? "is-selected" : ""}`}><strong>{item.name}</strong><span>{item.description}</span>{selected && <em className="training-selected-mark">SELECTED</em>}</button>; })}</div></section>
