...
const assignments = ["Endpoint", "Identity", "Cloud", "Network"] as const;
...
export default function IntrusionAnalysis({ onBack }: { onBack: () => void }) {
  ...
  const [selectedAssignments, setSelectedAssignments] = useState<Assignment[]>(["Endpoint"]);
  ...
  const start = (seed = seedFromNow()) => {
    setCaseData(generateIntrusionCase(seed, difficulty, recentFingerprints(), selectedAssignments));
    setStageIndex(0); setSelected(null); setHistory([]); setLives(3); setStreak(0); setBestStreak(0); setComplete(false);
  };

  const toggleAssignment = (item: Assignment) => {
    setSelectedAssignments((current) => current.includes(item)
      ? current.length === 1 ? current : current.filter((value) => value !== item)
      : [...current, item]);
  };
  ...
  if (!caseData) return <div className="training-shell space-y-6">
    ...
    <section className="training-card rounded-2xl p-5 sm:p-7">
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <button
          type="button"
          aria-pressed={selectedAssignments.length === assignments.length}
          onClick={() => setSelectedAssignments([...assignments])}
          className={`training-length ${selectedAssignments.length === assignments.length ? "is-selected" : ""}`}
        >
          All
        </button>
        {assignments.map((item) => (
          <button
            key={item}
            type="button"
            aria-pressed={selectedAssignments.includes(item)}
            onClick={() => toggleAssignment(item)}
            className={`training-length ${selectedAssignments.includes(item) ? "is-selected" : ""}`}
          >
            {item}
          </button>
        ))}
      </div>
      <p className="theme-muted mt-4 text-sm">Cases will be drawn from <b>{selectedAssignments.join(" + ")}</b>. Select multiple disciplines for blended investigations, or choose All for the full case pool.</p>
    </section>
  ...
  const choose = (answerId:string) => {
    ...
    const correct = Boolean(answer.correct); const lifeLost = !correct;
  ...
  const next = () => { if(lives===0||stageIndex===caseData.stages.length-1){remember(caseData.fingerprint);setComplete(true);}else{setStageIndex((value)=>value+1);setSelected(null);} };
  ...
  return <div className="training-shell space-y-5">
    ...
    <div className="grid gap-5 xl:grid-cols-[.9fr_1.1fr]">
      <aside className="training-card rounded-2xl p-5">
        <div className="flex flex-wrap items-end justify-between gap-2">
          <p className="training-section-title">EVIDENCE · {current.phase}</p>
          <p className="theme-muted text-[10px] font-bold tracking-[.1em]">{current.evidence.length} ARTIFACT{current.evidence.length===1?"":"S"} · REVIEW BEFORE DECIDING</p>
        </div>
        <div className="mt-4 space-y-3">{current.evidence.map((item,index)=><EvidenceCard key={item.id} item={item} index={index}/>)}</div>
        {discoveries.length>0&&<div className="mt-5 border-t border-current/10 pt-4"><p className="training-section-title">DISCOVERED</p><div className="mt-3 flex flex-wrap gap-2">{[...new Set(discoveries)].map((item)=><span className="intrusion-tag is-discovered" key={item}>{item}</span>)}</div></div>}
      </aside>
      <section className="training-card rounded-2xl p-5 sm:p-7"><p className="theme-kicker text-xs font-black tracking-[.18em]">STAGE {stageIndex+1} / {caseData.stages.length} · {current.phase}</p><h3 className="theme-title mt-3 text-xl font-black leading-relaxed sm:text-2xl">{current.question}</h3><div className="mt-6 grid gap-3">{current.answers.map((option)=>{const state=selected?option.correct?"is-correct":option.id===selected?"is-incorrect":"is-locked":"";return <button key={option.id} disabled={!!selected} onClick={()=>choose(option.id)} className={`training-answer ${state}`}><span>{option.id}</span><strong>{option.text}</strong></button>;})}</div>{selected&&selectedAnswer&&<div className={`training-feedback mt-6 ${selectedAnswer.correct?"is-correct":"is-incorrect"}`}><p className="text-sm font-black tracking-[.18em]">{selectedAnswer.correct?"STRONG DECISION":"INCORRECT DECISION — LIFE LOST"}</p><p className="theme-muted mt-3 leading-7">{selectedAnswer.explanation}</p>{lives===0&&<p className="mt-3 font-bold">No lives remain. The investigation has failed.</p>}<button className="training-primary mt-6" onClick={next}>{lives===0?"VIEW FAILED CASE REPORT →":stageIndex===caseData.stages.length-1?"FINAL ASSESSMENT →":"REVEAL NEXT EVIDENCE →"}</button></div>}</section>
    </div>
  </div>;
}
...
function EvidenceCard({item,index}:{item:IntrusionCase["stages"][number]["evidence"][number];index:number}) {
  const terminal = /powershell|process|edr|sysmon|command|waf|iis|dns/i.test(`${item.source} ${item.title}`);
  const path = /→|spawned|authenticated from|accessed .* from/i.test(item.detail);
  const lines = item.detail.split(/;\s*/).filter(Boolean);
  return <article className={`intrusion-evidence ${terminal?"is-terminal":path?"is-path":"is-record"}`}>
    <header className="intrusion-evidence-header">
      <div><span className="intrusion-evidence-index">{String(index+1).padStart(2,"0")}</span><p className="font-black">{item.source}</p></div>
      <time>{item.timestamp} UTC</time>
    </header>
    <div className="intrusion-evidence-title"><h4>{item.title}</h4><span>{terminal?"TERMINAL / LOG":path?"ACTIVITY PATH":"CASE RECORD"}</span></div>
    {terminal&&<div className="intrusion-terminal-bar"><i/><i/><i/><b>analyst-view</b></div>}
    <div className="intrusion-evidence-body">
      {terminal&&<span className="intrusion-prompt">$</span>}
      <code>{lines.map((line,lineIndex)=><span key={`${item.id}-${lineIndex}`}>{line}</span>)}</code>
    </div>
  </article>;
}
